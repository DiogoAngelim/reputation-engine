import type { ExecutivePublicLevel, ReputationEventType } from "../types/reputation.js";
...
const getInt = (input: unknown): number => {
  const parsed = Number(input);
  if (!Number.isFinite(parsed)) return 0;
  return Math.trunc(parsed);
};

const getBool = (input: unknown): boolean => input === true;

export const classifyPublicLevel = (reputationScore: number): ExecutivePublicLevel => {
  if (reputationScore >= 85) return "ELITE";
  if (reputationScore >= 70) return "SELECT";
  return "STANDARD";
};

export const normalizeTo100 = (value: number): number => round2(clamp(value, 0, 1) * 100);

export const calculatePriceTrendScore = (events: ReputationEventRecord[]): { score: number; avgClearingPrice: number } => {
...
export const calculateExecutiveScore = (events: ReputationEventRecord[]): ExecutiveScoreResult => {
  ...
  const openedSlots = events.reduce((sum, event) => {
    if (event.type !== "SLOT_UNFILLED") return sum;
  ...
  const reliabilityScore = round6(
    clamp(completionRate * 0.35 + onTimeRate * 0.25 - breachRate * 0.25 + slotFillRate * 0.15),
  );
...
export const calculateOwnerScore = (events: ReputationEventRecord[]): OwnerScoreResult => {
  ...
  const ownerBreaches = events.filter((event) => {
    if (event.type !== "CONTRACT_BREACH") return false;
  ...
  const internalRiskScore = normalizeTo100(
    clamp(
      (1 - paymentReliability) * 0.35 +
...
export const eventPayloadFlag = (payload: Record<string, unknown>, key: string): boolean =>
  getBool(payload[key]);
